...
#' @param param An \code{EpiModel} object of class \code{\link{param.adol}}.
#' @param init An \code{EpiModel} object of class \code{\link{init.adol}}.
#' @param control An \code{EpiModel} object of class \code{\link{control.adol}}.
...
#' @keywords module
#'
initialize.adol <- function(x, param, init, control, s) {
  ...
  # Sexual Debut
  dat$attr$has.debuted <- dat$nw[[1]] %v% "has.debuted"
  
  # AI.
  dat$attr$everAI <- rep(0,num)
  dat$attr$AI.adult.count<-rep(0,num)
  dat$attr$AI.adult.count.t<-rep(0,num)
  dat$attr$AI.ysmm.count<-rep(0,num)
  dat$attr$uaicount<-rep(0,num)

  
  #Timers
  dat$attr$has.debuted.time <-dat$nw[[1]] %v% "has.debuted"
  dat$attr$AI.time<-rep(0,num)
  dat$attr$active.time<-rep(0,num)
  dat$attr$prepStart.time<-rep(Inf,num)
  
    # Risk group
  dat$attr$riskg <- dat$nw[[1]] %v% "riskg"
...
#  dat$attr$status <- status
  ...
  #Limit infection status to debut and repopulate.
  status<-cbind(rep(0, num),dat$attr$has.debuted)
  nInf<-round(dat$init$prev.B*sum(dat$attr$has.debuted))


        for (i in 1:(length(status[,1]))){
        if (status[i,2]==0) status[i,1]<-0
        if (status[i,2]==1 & status[i,1]==0)  status[i,1]<-rbinom(1, 1, .07)}
 

  
  dat$attr$status <- status[,1]
  ...
  dat$attr$evertested<- evertested
