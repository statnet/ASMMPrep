...
test.adol <- function(dat, at) {
  ...
  if (testing.pattern == "memoryless") {
    elig.B <- which(active == 1 & race == "B" & tt.traj != "NN" & has.debuted ==1 &
    ...
    elig.W <- which(active == 1 & race == "W" & tt.traj != "NN" & has.debuted ==1 &
                    (diag.status == 0 | is.na(diag.status)))
  ...
  if (testing.pattern == "interval") {
    ...
    tst.B.nprep <- which(active == 1 & race == "B" & tt.traj != "NN" & has.debuted ==1 &
                         (diag.status == 0 | is.na(diag.status)) &
                         tsincelntst >= mean.test.B.int)
    tst.B.prep <- which(active == 1 & race == "B" & tt.traj != "NN" & has.debuted ==1 &
                        (diag.status == 0 | is.na(diag.status)) &
                        prepStat == 1 & tsincelntst >= prep.tst.int)
    tst.B <- c(tst.B.nprep, tst.B.prep)

    tst.W.nprep <- which(active == 1 & race == "W" & tt.traj != "NN" & has.debuted ==1 &
                         (diag.status == 0 | is.na(diag.status)) &
                         tsincelntst >= mean.test.W.int)
    tst.W.prep <- which(active == 1 & race == "W" & tt.traj != "NN" & has.debuted ==1 &
                        (diag.status == 0 | is.na(diag.status)) &
                        prepStat == 1 & tsincelntst >= prep.tst.int)
  ...
  dat$attr$evertested[tst.neg]<-1
  dat$attr$evertested[tst.pos]<-1
