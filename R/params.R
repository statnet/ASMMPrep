...
#'        each testing visit.
#' @param prep.delay The number of timesteps between becomeing eligible for PreP and starting PrEP.
#' @param prepSpell If \code{TRUE}, there is a constant hazard fro dropping PrEP.
#' @param PrepDrop The hazard for dropping PrEP given prepSpell==TRUE.
#' @param prep.uaicount.thresh The number of UAI one must have to initiate prep.
#' @param heat.prob.trans a matrix of dimensions length (riskg) by length(age) of the age and riskgroup 
...
#' @param heat.prob.contact a matrix of dimensions length (riskg) by length(age) of the age and riskgroup 
#'        specific probability of AI with a node class not in the simulation (heat bath). 
#' @param heat.trans the probability of transmision in the heatbath given contact.           
#' @param heat.multiplier is an atomic mutiplier to uniformly increase or devrease the intensity 
...
#'
#' @export
param.adol <- function(nwstats,
                       race.method = 1,    
                       last.neg.test.B.int = 2900,
                       last.neg.test.W.int = 2900,
                       mean.test.B.int = 2900,
                       mean.test.W.int = 2900,
                       ...
                       base.ai.main.BB.rate = 0.0384,
                       base.ai.main.BW.rate = 0.0384,
                       base.ai.main.WW.rate = 0.0384,
                       base.ai.pers.BB.rate = 0,
                       base.ai.pers.BW.rate = 0,
                       base.ai.pers.WW.rate = 0,
                       ai.scale = 3.6,
                       cond.main.BB.prob = 0.54,
                       cond.main.BW.prob = 0.54,
                       cond.main.WW.prob = 0.54,
                       ...
                       cond.diag.pers.beta = 0,
                       cond.discl.pers.beta = 0,
                       cond.diag.inst.beta = 0,
                       cond.discl.inst.beta = 0,
                       ...
                       prep.efficacy = 0.92,
                       prep.class.prob = c(.2085, .2438, .1314, .4163), 
                       prep.class.effect = c(0, 0.31, 0.81, 0.95),
                       prep.coverage = .4,
                       ...
                       prepSpell = FALSE,
                       prepDrop = .014337,
                       prep.uaicount.thresh = 10,
                       heat.prob.trans = matrix(c(0.0000010,	0.0000020,	0.0000030,	0.0000040,	0.0000050,	0.0000060,
                                            0.0000020,	0.0000040,	0.0000060,	0.0000080,	0.0000100,	0.0000121,
                                            0.0000030,	0.0000060,	0.0000090,	0.0000121,	0.0000151,	0.0000181,
                                            0.0000047,	0.0000093,	0.0000140,	0.0000187,	0.0000234,	0.0000280,
                                            0.0000147,	0.0000294,	0.0000441,	0.0000588,	0.0000735,	0.0000882
                       ), nrow = 5, ncol=6, byrow=TRUE),
                       heat.prob.contact=heat.prob.trans/(.5*.1*.01),
                       heat.trans = (.5*.1*.01),
                       heat.multiplier = 8.2,
                       ...) {
  ...
  class(p) <- c("param.adol", "param.net")
  return(p)
}
...
#' @param birth.age.prob Vector of length ages indicating the probability of entering the population at age.
#' @param debut.entry.prob probability of sexually debuing at entry.
#' @param debut.prob probability per timestep of debuing given not yet debuted.
#' @param birth.age.min the minimum age of a new entrant.
#' @param ... Additional arguments passed to function.
#'
#' @return
#' A list object of class \code{init.adol}, which can be passed to EpiModel
#' function \code{\link{netsim}}.
#'
#' @export
init.adol <- function(nwstats, prev.B = 0.15, prev.W = 0.15, ...) {
  ...
  p$birth.age.prob<-nwstats$birth.age.prob
  p$debut.entry.prob<-nwstats$debut.entry.prob
  p$debut.prob<-nwstats$debut.prob
  p$birth.age.min<-nwstats$birth.age.min

  p$nwstats <- NULL

  class(p) <- c("init.adol", "init.net")
  return(p)
}
...
#' @param debut.FUN Module function for sexual debut.
...
#' @param heatbath.FUN Module function to stochastically simulate disease transmission
#'        over age specific exposure probabilities to actors outside the simulation.
...
#'
#' @return
#' A list object of class \code{control.adol}, which can be passed to the
#' EpiModel function \code{netsim}.
#'
#' @export
control.adol <- function(simno = 1,
                         nsims = 1,
                         ncores = 1,
                         nsteps = 100,
                         start = 1,
                         resim.int = 1,
                         initialize.FUN = initialize.adol,
                         aging.FUN = aging.mard,
                         debut.FUN=debut.adol,
                         deaths.FUN = deaths.mard,
                         births.FUN = births.adol,
                         test.FUN = test.adol,
                         tx.FUN = tx.mard,
                         prep.FUN = prep.adol,
                         ...
                         edgescorr.FUN = edges_correct.adol,
                         ...
                         heatbath.FUN = heatbath.adol,
                         riskhist.FUN = riskhist.adol,
                         getprev.FUN = prevalence.adol,
  ...
  class(p) <- c("control.adol", "control.net")
  return(p)
}
